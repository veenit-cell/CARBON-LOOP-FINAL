...
1. On `/`, select **Enter player demo**.
2. On `/demo`, point out the visible **Hackathon Demo**, **Synthetic Data**, **Simulated Evidence**, and **Mock Reward** labels.
3. Select **Start & complete demo** and show the simulated mission result with separately reported Eco XP, Green Points, and avoided CO2e.
...
7. Return to `/demo` and select **Reset Demo**. It calls the explicit `SIMULATED_DEMO_ONLY` process-local reset endpoint and refetches the player and dashboard data.
